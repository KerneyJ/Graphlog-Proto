...
use openssl::{error::ErrorStack, hash::{Hasher, MessageDigest}, pkey::{PKey, Private, Public}, sign::Signer};
...
impl Reid {
    pub fn new(id: Id, pow: Option<Vec<u8>>, expiration: DateTime<Utc>, sig: Sig, claims: Option<Vec<(String, Key)>>, anchors: Option<Vec<(String, String)>>, revoked: bool) -> Self {
        Self {id, pow, expiration, sig, claims, anchors, revoked }
    }

    pub fn new_with_keys(pub_key: PKey<Public>, prv_key: PKey<Private>, expiration: DateTime<Utc>, pow: Option<Vec<u8>>, claims: Option<Vec<(String, Key)>>, anchors: Option<Vec<(String, String)>>, revoked: bool) -> Self {
        // Generate ID which is just hash(public key)
        let mut hasher = Hasher::new(MessageDigest::sha256()).unwrap();
        let pub_key_raw: Vec<u8> = match pub_key.raw_public_key() {
            Err(why) => panic!("Couldn't convert public key into raw bytes using raw_public_key: {why}"),
    ...
    pub fn append_anchor(&mut self, anchor_type: String, anchor_value: String) {
        self.anchors.get_or_insert_with(Vec::new).push((anchor_type, anchor_value));
    }

    pub fn append_claim(&mut self, key_type: String, key_value: Key) {
        self.claims.get_or_insert_with(Vec::new).push((key_type, key_value));
    }
    ...
    pub fn update_sig(&mut self, prv_key: PKey<Private>) -> std::result::Result<Sig, ErrorStack> {
        Reid::sign_reid(prv_key, &self.id, self.expiration, &self.claims, &self.anchors)
    }
